 ...
 * Usage: node scripts/assign-questionnaires-to-patient.mjs <patientEmail>
 */
...
async function assignQuestionnairesToPatient(patientEmail) {
  try {
    ...
    if (existingQuestionnaires.size > 0) {
      ...
      console.log('\n❓ Voulez-vous quand même assigner les questionnaires par défaut ?');
      console.log('   (Les questionnaires seront ajoutés, les existants ne seront pas modifiés)');
    }
...
if (!patientEmail) {
  console.error('❌ Usage: node scripts/assign-questionnaires-to-patient.mjs <patientEmail>');
  console.error(
    '   Exemple: node scripts/assign-questionnaires-to-patient.mjs martialcayre@live.fr'
  );
  process.exit(1);
}
